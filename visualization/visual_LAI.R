#!/usr/bin/R
#可视化LAI在染色体上的分布的代码
#Usage:Rscript visual_LAI.R genome.*.out.LAI species
#第一个参数是输出的LAI文件，注意需要自己删除包含contig或scaffold的行。只能用于染色体
#第二个参数是用于输出的文件名的前缀
# 输入文件是 genome.str15.chr.fa.out.LAI species
if ( length(args)==0 | args[1] == "-h" | args[1] == "--help" ){
  stop("Usage: visual_LAI.R genome.*.out.LAI species")
}
...
ggsave(paste0(species,".LAI.pdf"),dpi = 300)
ggsave(paste0(species,".LAI.png"),dpi = 300)
print(paste0("Output LAI image is ",species,".LAI.png"))
dev.off()
# 
# head(data2) 
